...
CreateToggle(combatPanel, "Visible Only (Wallcheck)", NovusHub.Configurations.Aimbot.WallCheck, function(state)
	NovusHub.Configurations.Aimbot.WallCheck = state
end)
CreateToggle(combatPanel, "Velocity Prediction", NovusHub.Configurations.Aimbot.Prediction, function(state)
...
-- Input Listeners with Chat Suppression Check
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	if gameProcessed or UserInputService:GetFocusedTextBox() then return end
	
	if input.KeyCode == NovusHub.Configurations.Aimbot.Keybind or 
	   (NovusHub.Configurations.Aimbot.AllowRightClick and input.UserInputType == Enum.UserInputType.MouseButton2) then
